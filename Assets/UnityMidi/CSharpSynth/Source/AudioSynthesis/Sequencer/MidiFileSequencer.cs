    ...
    public class MidiFileSequencer
    {
        ...
        private void LoadMidiFile(MidiFile midiFile)
        {
            ...
            for (int x = 0; x < mdata.Length; x++)
            {
                ...
                mdata[x].originalDelta = mEvent.DeltaTime;
                mdata[x].originalTrack = mEvent.track;
                //Update tempo
                ...
                mdata[x].BPM = BPM;
                //Debug.Log($"original delta: {mEvent.DeltaTime}, abs delta: {synth.SampleRate * mEvent.DeltaTime * (60.0 / (BPM * midiFile.Division))}");
            }
